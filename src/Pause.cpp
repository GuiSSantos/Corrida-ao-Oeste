...
Pause::Pause(float width, float height)
{
	...
	menu[0].setPosition(sf::Vector2f(width / 2 -30, height / (ITENS_MENU_P+4 ) * 1));

	menu[1].setFont(font);
	menu[1].setColor(sf::Color::White);
	menu[1].setString("Salvar");
	menu[1].setPosition(sf::Vector2f(width / 2-30, height / (ITENS_MENU_P+4 ) * 2));

	menu[2].setFont(font);
	menu[2].setColor(sf::Color::White);
	menu[2].setString("Sair");
	menu[2].setPosition(sf::Vector2f(width / 2-25, height / (ITENS_MENU_P+4 ) * 3));
...
void Pause::MoveDown()
{
	if (selectedItemIndex + 1 < ITENS_MENU_P)
	{
		menu[selectedItemIndex].setColor(sf::Color::White);
		selectedItemIndex++;
		menu[selectedItemIndex].setColor(sf::Color::Red);
